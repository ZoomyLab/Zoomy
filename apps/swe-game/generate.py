...
def per_tag_bc_kernels(model, printer):
    """Codegen one kernel per boundary tag — ``bc_<tag>`` — instead of
    the monolithic Piecewise dispatch.

    Reaches into ``model.boundary_conditions.boundary_conditions_list``
    and builds a standalone Function for each tag's
    ``compute_boundary_condition``, then runs the existing
    ``_generate_bc_kernel`` printer over it.  The solver can then call
    a single cheap kernel per tag — no symbolic Piecewise, no fat
    if/else on the hot wall-face path.  ``printer`` is a JsModel /
    GlslModel instance (its symbol maps are already registered).

    Returns ``(code, tag_order)`` — ``tag_order`` is the sorted tag
    list, so index i ↔ ``bc_<tag_order[i]>``.
    """
    m = model
    blocks = []
    tags = []
...
def main():
    ...
    js_bc, bc_tags = per_tag_bc_kernels(model, JsModel(model))
    js = "\n\n".join([
    ...
    glsl_bc, _ = per_tag_bc_kernels(model, GlslModel(model))
    glsl = "\n\n".join([
        "// AUTO-GENERATED by apps/swe-game/generate.py — do not edit.",
