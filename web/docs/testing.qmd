...
Use runtime markers to run only tests valid for your local/container setup:

```bash
# core-only fast tests
pytest tests -m "small and core"

# jax fast tests
pytest tests -m "small and jax"

# amrex/petsc/firedrake fast tests
pytest tests -m "small and amrex"
pytest tests -m "small and petsc"
pytest tests -m "small and firedrake"
```

When adding new tests, assign both:

- a runtime marker (`core`, `jax`, `amrex`, `petsc`, or `firedrake`)
- a scale/intention marker (`small`, `large`, `benchmark`, `tutorial`, or `regression`)

For tutorial smoke tests, this means at least:

- `@pytest.mark.tutorial`
- one runtime marker, e.g. `@pytest.mark.core`
