...
export class ZoomyCLI {
    ...
    vizPrelude() {
        return [
            ...
            "",
            "# The Run cell above already opened its own output: every solver card",
            "# ends with open_hdf5('/tmp/zoomy_sim/sim.h5'), which publishes `store`.",
            "# Reuse it. Reading simulation.h5 unconditionally here overwrote that",
            "# store with a file no solver card writes -- the name comes from the",
            "# session-packing scripts -- so a run that had just finished with 16",
            "# snapshots died in the plot with FileNotFoundError. Fall back to",
            "# simulation.h5 for a packed session that ships one, and to None for a",
            "# case whose deliverable is not a field at all (the code-printer session",
            "# emits an AMReX header and its viz card never touches the store).",
            "store = globals().get(\"store\")",
            "if store is None and os.path.exists(\"simulation.h5\"):",
