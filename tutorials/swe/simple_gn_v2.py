...
class GreenNaghdiLikeModelV2(StructuredDerivativeModel):
    """
    Minimal GN-inspired model:
      Q = [h, hu]
      source_2 includes alpha * d_txx(h)
    """

    dimension = 1
    variables = ["h", "hu"]
    parameters = {"g": (9.81, "positive"), "alpha": (0.02, "positive")}
    ...
    def source(self):
        ...
        S[1] = self.params.alpha * self.D.dtxx(self.Q.h)
        return S
...
def make_model():
    ...
    def ic_func(x):
        ...
        Q[0] = 1.0 + 0.05 * np.exp(-((x[0] - 5.0) ** 2) / 0.5)
        Q[1] = 0.0
        return Q

    model = GreenNaghdiLikeModelV2(
...
def run():
    ...
    settings = Zstruct(
        output=Zstruct(
            directory="outputs/tutorial_gn_v2",
            filename="gn_v2",
            snapshots=2,
    ...
    plot_path = "outputs/tutorial_gn_v2/gn_v2_h_u.png"
